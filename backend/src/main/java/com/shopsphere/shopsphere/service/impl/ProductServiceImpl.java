...
public class ProductServiceImpl implements ProductService {
    ...
    @Override
    @Transactional
    @CacheEvict(value = "products", allEntries = true) // new product -> stale list/search caches, wipe them
    ...
    @Override
    // Cache individual product lookups by id — this is the hot read path (product detail page)
    ...
    @Transactional
    @Caching(evict = {
            @CacheEvict(value = "product", key = "#id"),      // this product's own cached detail entry
            @CacheEvict(value = "products", allEntries = true) // any list/search page might contain it -> wipe all
    })
